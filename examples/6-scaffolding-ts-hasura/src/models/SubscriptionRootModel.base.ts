...
export class SubscriptionRootModelSelector extends QueryBuilder {
  ...
  polls(builder: string | PollsModelSelector | ((selector: PollsModelSelector) => PollsModelSelector) | undefined, args?: { distinctOn?: PollsSelectColumn[] | null, limit?: (number | null), offset?: (number | null), orderBy?: PollsOrderBy[] | null, where?: (PollsBoolExp | null) }) { return this.__child(`polls`+ (args ? '('+['distinctOn', 'limit', 'offset', 'orderBy', 'where'].map((argName) => ((args as any)[argName] ? `${argName}: ${JSON.stringify((args as any)[argName])}` : null) ).filter((v) => v!=null).join(', ') + ')': ''), PollsModelSelector, builder) }
