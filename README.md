...
- `RootStore.Model` - The store generated by mst-gql that exposes the following stuff
...
- `XXX.model` classes per type found in the graphQL endpoint. These expose the following things:
...
For the root store and every grapqhl object type, two files will be generated. For example `Todo.model.js` and `Todo.js`. The `Todo.model.js` file holds all the generated code, and will be replaced on every next run of `mst-gql`. The `Todo.js` file defines the model you will actually be using throughout your application.
